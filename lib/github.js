...
function parseBlogPost(repo, rawMarkdown, blogJson = null) {
  ...
  } else if (typeof rawReadingTime === 'string' && rawReadingTime.trim()) {
    readingTime = /min/i.test(rawReadingTime) ? rawReadingTime.trim() : `${rawReadingTime.trim()} min read`;
  } else {
    readingTime = calculateReadingTime(resolvedContent);
  }
