...
const localBlogs = require('./local-blogs.json');

const GITHUB_API_URL = 'https://api.github.com';

// Cache structure for in-memory caching during ISR / SSR
const cache = {
  repos: null,
  reposTimestamp: 0,
  blogs: new Map(),
...
/**
 * Determine if a repository is intended to be a blog post
 */
function isBlogRepo(repo) {
  if (!repo || !repo.name) return false;
  const name = repo.name.toLowerCase();

  // 1. Naming convention: blog-* or blog_*
  if (name.startsWith('blog-') || name.startsWith('blog_') || name.endsWith('-blog')) {
    return true;
  }

  // 2. GitHub topics: blog, article, post, insights
  if (Array.isArray(repo.topics)) {
    const validTopics = ['blog', 'article', 'post', 'insights', 'noc-blog'];
    if (repo.topics.some((t) => validTopics.includes(t.toLowerCase()))) {
      return true;
    }
  }

  // 3. Description tag e.g. [blog]
  if (repo.description && (repo.description.toLowerCase().includes('[blog]') || repo.description.toLowerCase().includes('(blog)'))) {
    return true;
  }
 ...
 */
async function fetchAccountRepos(owner) {
  const now = Date.now();
  if (cache.repos && now - cache.reposTimestamp < cache.CACHE_TTL_MS) {
    return cache.repos;
  }
...
/**
 * Fetch raw README and optional blog.json for a repository
 */
async function fetchRepoBlogContent(owner, repoName, defaultBranch = 'main') {
  const branch = defaultBranch || 'main';
  const readmeCandidates = ['README.md', 'readme.md', 'README.MD', 'Readme.md', 'README', 'readme'];

  let markdown = null;

  // Try raw fetching README for maximum speed and simplicity
  ...
  if (!markdown) {
    try {
      const fileData = await githubFetch(`/repos/${owner}/${repoName}/readme`);
...
function parseBlogPost(repo, rawMarkdown, blogJson = null) {
  ...
  // Merge optional blog.json
  if (blogJson && typeof blogJson === 'object') {
    frontmatter = { ...frontmatter, ...blogJson };
  }

  // Extract title
  let title = frontmatter.title || frontmatter.headline;
  if (!title) {
    const firstHeading = content.match(/^#\s+(.+)$/m);
    if (firstHeading) {
      title = firstHeading[1].trim();
      // Remove first heading from body to avoid duplication in page header
      content = content.replace(/^#\s+.+$/m, '').trim();
    } else {
      title = formatSlugToTitle(repo.name);
    }
  }

  // Extract description / excerpt
  let description = frontmatter.description || frontmatter.excerpt || repo.description;
  if (!description) {
    ...
    description = paragraphs[0] ? paragraphs[0].slice(0, 160) + '...' : 'Insights and operations guide from NOC247.';
  }

  // Dates
  const datePublished = frontmatter.date || frontmatter.datePublished || frontmatter.publishedAt || repo.created_at || new Date().toISOString();
  const dateModified = frontmatter.dateModified || frontmatter.updatedAt || repo.pushed_at || datePublished;
  ...
  // First image in content or frontmatter
  let image = frontmatter.image || frontmatter.coverImage || frontmatter.ogImage;
  if (!image) {
  ...
  // Resolve hero image if relative
  ...
  const slug = frontmatter.slug || repo.name;
...
async function fetchAllBlogs() {
  const now = Date.now();
  if (cache.allBlogs && now - cache.allBlogsTimestamp < cache.CACHE_TTL_MS) {
  ...
  // 1. Add local fallback blogs
  for (const lb of localBlogs) {
    allBlogsMap.set(lb.slug, {
      ...lb,
      url: `https://noc247.io/blog/${lb.slug}`,
      isGitHub: false,
    });
  }

  // 2. Fetch and discover GitHub blogs
  try {
    const repos = await fetchAccountRepos(owner);
    ...
    for (const blog of parsedGithubBlogs) {
      if (blog && blog.slug) {
        // GitHub version overrides or adds new blog
        allBlogsMap.set(blog.slug, blog);
        // Also map stripped prefix if slug has blog-
        if (blog.slug.startsWith('blog-')) {
          allBlogsMap.set(blog.slug.replace(/^blog-/, ''), blog);
        }
      }
    }
  } catch (err) {
    console.warn('[GitHub Blog] Discovery failed, serving local fallback blogs:', err.message);
  }
...
async function fetchBlogBySlug(slug) {
  ...
  // 3. Check local blogs as fallback
  const localMatch = localBlogs.find(
    (b) => b.slug.toLowerCase() === cleanSlug || `blog-${b.slug.toLowerCase()}` === cleanSlug
  );
  if (localMatch) {
    return {
      ...localMatch,
      url: `https://noc247.io/blog/${localMatch.slug}`,
      isGitHub: false,
    };
  }
