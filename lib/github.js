  ...
  try {
    ...
    for (const blog of parsedGithubBlogs) {
      if (blog && blog.slug) {
        ...
        githubBlogsFound = true;
      }
    }
  } catch (err) {
    console.warn('[GitHub Blog] Discovery failed, checking local fallback:', err.message);
  }

  // 2. Only use local fallback blogs if no GitHub blogs could be loaded
  if (!githubBlogsFound) {
